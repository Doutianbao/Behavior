...
for block = 1:numel(blockInds)-1
    ...
    if ~isempty(bp)
        ...
        %             'lineLen',lineLen);
        fp = GetFishPos(im_proc,nPxls,'filter',bp,'process','parallel');
    else
        %         [fp,hOr_temp] = GetFishPos(im_proc, 25,'process','parallel','lineLen',lineLen);
        fp = GetFishPos(im_proc,nPxls,'process','parallel');
    end
...
%     tic
%     [~,hOr_crop] =  GetFishPos(IM_proc_crop, 25,'filter',bp,'process','parallel','lineLen',lineLen);
%     toc
% end
...
% procData.hOr_crop = hOr_crop; % No longer computing head orientation from
%                                 script that also gets fish position
