...
for imgNum = 1:size(IM,3)
    ...
    for jj = 1:length(heights)
        base = round(heights(jj)/5);
        T = CreateTailTemplate(base,heights(jj),-0.2);
        if jj ==1            
            lineInds{jj} = GetML(img,startPt,[],5,heights(jj),T);
        else
            try
            lineInds{jj} = GetML(img,startPt,prevStartPt,5,heights(jj),T);
            catch
                a = 1;
            end
        end
