function midlineInds = GetMidlines_vr(IM,varargin)
...
% 'ref' - refImg, where refImg is the reference image computed by taking the mean (or median or mode)
%       of all images. Used here for finding indices outside the fish arena to set them to values
%       that minimize drawing a line segment over them instead of over the
%       fish when the fish is close to the edge of the arena
% 
% Outputs:
